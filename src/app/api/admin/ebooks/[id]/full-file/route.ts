...
import { uploadFullEbookFile } from "@/lib/storage";
...
export async function POST(request: Request, { params }: RouteParams) {
  const rateLimit = await checkRateLimit(`upload-full-ebook:${getClientIp(request)}`, {
    limit: 20,
    windowSeconds: 10 * 60,
  });
  ...
  const uploadResult = await uploadFullEbookFile(ebook.slug, file);
  ...
  const { data: updatedEbook, error: updateError } = await supabase
    .from("ebooks")
    .update({ private_file_path: uploadResult.path, updated_at: new Date().toISOString() })
    .eq("id", id)
    .select("*")
    .maybeSingle();
